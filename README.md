...
> 📌 **项目定位**：个人维护的开源安全工具，面向**授权**的安全测试与学习场景。核心功能（资产→扫描→漏洞→报告闭环）经端到端自动化测试验证，稳定性按"内网/单机使用"目标设计；分布式扫描、插件市场等高级能力为路线图预留项，欢迎通过 Issue / PR 参与改进。
...
**Windows 一键启动**（推荐本地体验）：双击 `start_all.bat`（自动启动后端 + 靶场 + 打开浏览器）；停止用 `stop_all.bat`；服务掉线自动拉起可运行 `watchdog.ps1`。
